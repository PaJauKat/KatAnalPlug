...
import com.example.PajauApi.BreakHandler.ReflectBreakHandler;
...
@PluginDescriptor(
        name = "<html>[<font color=\"#FA4444\">P</font>] Rooftop Agility</html>",
        description = "Partially ported iRooftops (by illumine)",
        enabledByDefault = false,
        tags = {"ethan", "piggy", "pajau"}
)
...
public class RooftopAgilityPlugin extends Plugin {
    ...
    private void eatFood() {
        Optional<Widget> cake = Inventory.search().matchesWildCardNoCase(config.foodName()).first();
        if (cake.isPresent()) {
    ...
    private boolean shouldEatSummerPie() {
        return config.boostWithPie() &&
                ...
                (Inventory.getItemAmount(ItemID.SUMMER_PIE) > 0 || Inventory.getItemAmount(ItemID.SUMMER_PIE) > 0);
    }
    ...
    private boolean shouldRestock() {
        ...
        return Inventory.search().matchesWildCardNoCase(config.foodName()).empty();
    }

    private void restockItems() {
        if (Bank.isOpen()) {
            if (!config.foodName().isEmpty() && Inventory.search().matchesWildCardNoCase(config.foodName()).empty()) {
                Optional<Widget> bankFood = Bank.search().matchesWildCardNoCase(config.foodName()).first();
            ...
            if (config.boostWithPie() && BankInventory.search().withId(ItemID.SUMMER_PIE).empty()) {
